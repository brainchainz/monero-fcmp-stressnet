...
function WalletTab({ onExplorerTx }) {
    ...
    const sendMax = () => {
        const unlocked = balance?.unlocked_balance || 0;
        const reserve = 1000000000; // 0.001 tXMR fee headroom
        const amt = Math.max(0, unlocked - reserve) / 1e12;
        setSendAmt(amt > 0 ? amt.toFixed(6) : '0');
    };

    const [rescanning, setRescanning] = useState(false);
    const rescanWallet = async (mode) => {
        if (mode === 'blockchain' && !window.confirm(
            'Full rescan re-reads the entire chain from the wallet restore height. On the stressnet this takes a few minutes and the wallet is unusable until it finishes. Continue?')) return;
        setRescanning(true);
        setError(null);
        try {
            await axios.post(`${API}/wallet/rescan`, { mode }, { timeout: 20000 });
            // The rescan runs as a background job; poll status until it clears busy.
            fetchWallet({ force: true });
        } catch (e) {
            setError('Rescan failed: ' + (e.response?.data?.details || e.response?.data?.error || e.message));
        } finally {
            setRescanning(false);
        }
    };
    ...
    return (
        <div className="wallet-layout">
            ...
            <div className="wallet-body-row">
                <div className="glass-panel send-panel"><h3 className="panel-title"><Icons.Send size={14} color="var(--accent)" /> Send FCMP++ TX</h3><div className="send-form"><div className="form-group"><label>Recipient</label><input type="text" className="input-dark" placeholder="Stressnet address..." value={sendAddr} onChange={e => setSendAddr(e.target.value)} /></div><div className="form-row"><div className="form-group" style={{ flex: 1 }}><label>Amount (tXMR)</label><div className="addr-row"><input type="number" step="0.0001" className="input-dark" placeholder="0.0000" value={sendAmt} onChange={e => setSendAmt(e.target.value)} style={{ flex: 1 }} /><button className="btn-ghost btn-sm" onClick={sendMax} disabled={!balance?.unlocked_balance} title="Fill with your unlocked balance minus a fee reserve">Max</button></div></div></div><button className="btn-primary btn-full" onClick={sendTx} disabled={sending || !sendAddr || !sendAmt || !balance?.unlocked_balance}>{sending ? 'Broadcasting...' : 'Send Transaction'}</button><div className="mono-xs dimmed" style={{ marginTop: 6 }}>All transactions use FCMP++ proofs and are relayed over Tor.</div>{txResult && <div className={`tx-result ${txResult.success ? 'tx-ok' : 'tx-fail'}`}>{txResult.success ? <>TX Sent: <span className="mono-xs">{txResult.hash?.substring(0, 24)}...</span></> : <>Failed: {txResult.error}</>}</div>}</div></div>
